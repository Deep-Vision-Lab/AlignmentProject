...
def test_vit_emits_one_token_per_existing_sliding_window():
    ...
    with torch.no_grad():
        contextual, local, grouped, ink = model(image, return_local=True, return_grouped=True, return_ink=True)
...
def test_vit_heads_must_divide_embedding_dimension():
    try:
        EmbeddingModel(vector_size=62, vit_heads=4, device="cpu")
...
def test_evaluation_reconstructs_vit_from_checkpoint_config(tmp_path):
    ...
    torch.save({"image_model_state_dict": model.state_dict(), "model_config": {"visual_encoder_type": "vit", "window_size": 32, "stride": 16, "vector_size": 64, "lang": "Arabic", "use_bilstm": False, "use_local_window_grouping": False, "vit_input_height": 128, "vit_layers": 2, "vit_heads": 4, "vit_mlp_dim": 128, "vit_dropout": 0.0, "vit_max_tokens": 128}}, checkpoint_path)
    install_vit_evaluation_loader()
    from Evaluation import _eval_utils
    loaded = _eval_utils.load_evaluation_models(checkpoint_path, device="cpu", load_text_model=False)
