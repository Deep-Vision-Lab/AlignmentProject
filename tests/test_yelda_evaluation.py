...
def make_checkpoint(cross=False):
    config = dict(window_cnn_enabled=True, visual_encoder_type="vit", window_size=32,
    ...
    model = EmbeddingModel(device="cpu", use_flip=True, window_cnn_enabled=True,
...
def test_missing_cnn_weights_rejected():
    checkpoint, _, _ = make_checkpoint()
    key = next(key for key in checkpoint["model_state_dict"] if "patch_embedding.features.0.weight" in key)
