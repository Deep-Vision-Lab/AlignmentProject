...
def _unit_embeddings(text, text_unit_type, checkpoint, device):
    if text_unit_type == "ngram":
        tokenizer = load_ngram_tokenizer_if_available(checkpoint)
        if tokenizer is None:
            raise RuntimeError("fig09 --text_unit_type ngram requires ngram_vocab.json next to the checkpoint.")
        embedder = load_text_embedder(device)
        return encode_text_units(text, "ngram", embedder, device, ngram_tokenizer=tokenizer)
    chars = list(text)
    spans = [(idx, idx + 1) for idx in range(len(chars))]
    return chars, spans, _text_embeddings(chars, checkpoint, device)
...
@torch.no_grad()
def collect_pooled_unit_embeddings(
    model, checkpoint, num_samples, device, data_dir=None, text_unit_type="ngram"
):
    vectors, labels, metadata = [], [], []
    for sample_idx in range(num_samples):
        image, text = _sample(data_dir, sample_idx)
        text_padded = pad_text(text)
        units, spans, text_emb = _unit_embeddings(text_padded, text_unit_type, checkpoint, device)
        visual_emb = normalize_func(model(image.unsqueeze(0).to(device)).squeeze(0).float())
        result = compute_d3tw_char_pool_for_sample(
            visual_emb=visual_emb,
            text_emb=text_emb,
        ...
        metadata.extend([
            {
                ...
                "unit_idx": int(j),
                "token": units[j],
                "span": [int(spans[j][0]), int(spans[j][1])],
                "assigned_windows": [int(i) for i in result["groups"][j]],
                "num_windows": int(len(result["groups"][j])),
            }
...
def draw_embedding_space(
    ...
    if embedding_level in {"pooled_char", "pooled_token"}:
        if embedding_level == "pooled_token" or text_unit_type == "ngram":
            vectors, labels, metadata = collect_pooled_unit_embeddings(
                model, checkpoint, num_samples, device,
                data_dir=data_dir, text_unit_type=text_unit_type,
            )
            title = f"D3TW-pooled visual {text_unit_type} token vectors"
            color_label = f"true {text_unit_type} token"
            stem = f"fig09_embedding_space_pooled_{text_unit_type}"
        else:
        ...
        raise ValueError(f"Unknown embedding_level: {embedding_level}")
    ...
    scatter = ax.scatter(
        ...
        c=color_ids,
        cmap="tab20",
        s=14 if embedding_level in {"pooled_char", "pooled_token"} else 8,
        alpha=0.75,
        linewidths=0,
    )
...
def main():
    ...
    parser.add_argument("--embedding_level", default="pooled_char", choices=["window", "pooled_char", "pooled_token"])
