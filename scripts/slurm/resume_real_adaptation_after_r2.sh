#!/usr/bin/env bash
# Resume conservative real adaptation from an already-trained R2 checkpoint.
# Re-runs only R2 evaluation/gates, then final N=100 if the scientific gates pass.
set -euo pipefail
...
for required in "${R2_CHECKPOINT}" "${PREPROCESS_ENV}" "${EVAL_SWEEP}" "${GATE}" "${SUMMARY}"; do
  [[ -f "${required}" ]] || { echo "ERROR: missing required file ${required}" >&2; exit 2; }
done
for required_dir in "${R0_ROOT}" "${R1_ROOT}" "${PHASE3_ROOT}"; do
...
for name in eval_gate_real_r2 final_real_adaptation_eval; do
  if squeue -h -u "${USER}" -n "${name}" | grep -q .; then
    echo "ERROR: active job already exists with name ${name}. Cancel stale jobs first." >&2
    exit 2
  fi
done
...
echo "Resuming from R2 checkpoint: ${R2_CHECKPOINT}"
...
cat <<EOF
Submitted evaluation-only recovery from existing R2 checkpoint:
...
No training is repeated. Final evaluation starts only if R2 passes both scientific gates.
Do not switch branches or change tracked files in ${PROJECT_DIR} until these jobs finish.
EOF
