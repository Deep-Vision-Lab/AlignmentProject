...
LINE_WIDTH="${LINE_WIDTH:-1024}"; WINDOW_SIZE="${WINDOW_SIZE:-32}"; STRIDE_RATIO="${STRIDE_RATIO:-0.25}"; WINDOW_OVERLAP_MODE="${WINDOW_OVERLAP_MODE:-custom}"
read -r STRIDE_PIXELS IMAGE_WINDOWS < <(python - "${LINE_WIDTH}" "${WINDOW_SIZE}" "${STRIDE_RATIO}" "${WINDOW_OVERLAP_MODE}" <<'PY'
import sys
w=int(sys.argv[1]); k=int(sys.argv[2]); r=float(sys.argv[3]); m=sys.argv[4].strip().lower()
if w<=0 or k<=0 or k>w: raise SystemExit("Invalid line/window geometry")
...
(( IMAGE_WINDOWS >= EXPECTED_MAX_TEXT_CHARS )) || { echo "ERROR: ${IMAGE_WINDOWS} image windows cannot cover ${EXPECTED_MAX_TEXT_CHARS}-character lines. Use WINDOW_SIZE=32 and STRIDE_RATIO=0.25." >&2; exit 2; }
...
export NCCL_P2P_DISABLE NCCL_SHM_DISABLE NCCL_ASYNC_ERROR_HANDLING NCCL_DEBUG CUDA_DEVICE_ORDER WINDOW_SIZE STRIDE_RATIO WINDOW_OVERLAP_MODE
