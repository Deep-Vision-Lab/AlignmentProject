...
# Run from any repository branch with:
...
if [[ -z "${BRANCH_NAME}" ]]; then
  BRANCH_NAME="$(basename "${PROJECT_DIR}")"
fi
...
if (( PAIR_COUNT < NUM_SAMPLES )); then
  echo "ERROR: requested ${NUM_SAMPLES} pairs, but found only ${PAIR_COUNT} in ${DATA_DIR}." >&2
  exit 2
fi

# Augmentations were already applied while generating the PNG files.
...
printf '%s\n' \
  ...
  "  project            = ${PROJECT_DIR}" \
  "  dataset            = ${DATA_DIR}" \
  "  detected pairs     = ${PAIR_COUNT}" \
  "  training samples   = ${NUM_SAMPLES}" \
  "  epochs             = ${EPOCHS}" \
  "  online augmentation= disabled" \
  "  job id             = ${JOB_ID}"

# Connected-subword and stroke-aware branches already provide the canonical
# self-submitting synthetic launcher. Delegate while overriding its old defaults.
...
# Older and baseline branches use either the generic train.py entry point or
# the legacy run_train.sh wrapper.
if [[ -f "${PROJECT_DIR}/train.py" ]] \
  ...
  echo "ERROR: no supported training entry point was found on branch ${BRANCH_NAME}." >&2
  exit 2
fi
...
if [[ -z "${SLURM_JOB_ID:-}" ]]; then
  sbatch \
    ...
    --mail-type=ALL \
    --mail-user="${MAIL_USER}" \
    --export=ALL,PROJECT_DIR="${PROJECT_DIR}",DATA_DIR="${DATA_DIR}",JOB_ID="${JOB_ID}",NUM_SAMPLES="${NUM_SAMPLES}",EPOCHS="${EPOCHS}",NUM_GPUS="${NUM_GPUS}",TRAINING_MODE="${TRAINING_MODE}",SYNTHETIC_MANUSCRIPT_AUGMENT=0,REAL_AUGMENT=0,DATASET_TYPE=synthetic \
    "${SCRIPT_PATH}"
  exit 0
fi
...
if ! command -v conda >/dev/null 2>&1; then
  echo "ERROR: conda is unavailable on the compute node." >&2
  exit 2
fi
