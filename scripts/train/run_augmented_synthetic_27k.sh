...
printf '%s\n' "Augmented Arabic synthetic training" "  branch=${BRANCH_NAME}" "  dataset=${DATA_DIR}" "  pairs=${PAIR_COUNT}" "  samples=${NUM_SAMPLES}" "  epochs=${EPOCHS}" "  online augmentation=disabled" "  NCCL P2P=disabled" "  job id=${JOB_ID}"
if [[ -f "${PROJECT_DIR}/scripts/train/run_connected_subword_synthetic.sh" ]]; then NUM_GPUS="${NUM_GPUS:-2}"; exec bash "${PROJECT_DIR}/scripts/train/run_connected_subword_synthetic.sh"; fi
