...
[[ "${MAX_TEXT_SPAN_CHARS}" =~ ^[1-9][0-9]*$ ]] || { echo "ERROR: MAX_TEXT_SPAN_CHARS must be a positive integer." >&2; exit 2; }
[[ "${SPAN_MAX_CORE_CHARS_CAP}" =~ ^[0-9]+$ ]] || { echo "ERROR: SPAN_MAX_CORE_CHARS_CAP must be a non-negative integer." >&2; exit 2; }
...
texts_dir = pathlib.Path(sys.argv[1]); limit = int(sys.argv[2]); max_chars = int(sys.argv[3])
pattern = re.compile(r"text[12]_(\d+)\.txt$")
worst = (-1, -1, ""); seen = 0
for path in texts_dir.glob("text[12]_*.txt"):
    match = pattern.fullmatch(path.name)
    if not match or int(match.group(1)) > limit: continue
    text = path.read_text(encoding="utf-8").strip()
    required = 0; index = 0
    while index < len(text):
        if text[index].isspace(): required += 1; index += 1; continue
        end = index + 1
        while end < len(text) and not text[end].isspace(): end += 1
        required += math.ceil((end - index) / max_chars); index = end
    seen += 1
    candidate = (required, len(text), path.name)
    if candidate > worst: worst = candidate
if seen == 0: raise SystemExit(f"No transcript files found under {texts_dir}")
...
export WINDOW_SIZE STRIDE_RATIO WINDOW_OVERLAP_MODE MAX_TEXT_SPAN_CHARS SPAN_MAX_CORE_CHARS_CAP
