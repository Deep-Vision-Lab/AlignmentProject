...
if m=='no_overlap': s=k
elif m=='light_overlap': s=max(1,k//2)
elif m=='dense_overlap': s=max(1,k//4)
elif m=='custom': s=max(1,int(k*r))
else: raise SystemExit(f'Unknown WINDOW_OVERLAP_MODE={m!r}')
print(s,((w-k)//s)+1)
PY
)
...
export NCCL_P2P_DISABLE NCCL_SHM_DISABLE NCCL_ASYNC_ERROR_HANDLING NCCL_DEBUG CUDA_DEVICE_ORDER WINDOW_SIZE STRIDE_RATIO WINDOW_OVERLAP_MODE MAX_TEXT_SPAN_CHARS SPAN_MAX_CORE_CHARS_CAP
