...
BRANCH_NAME="$(git branch --show-current 2>/dev/null || true)"; [[ -n "${BRANCH_NAME}" ]] || BRANCH_NAME="$(basename "${PROJECT_DIR}")"
...
[[ "${PAIR_COUNT}" =~ ^[0-9]+$ ]] || { echo "ERROR: failed to count pairs" >&2; exit 2; }
(( PAIR_COUNT >= NUM_SAMPLES )) || { echo "ERROR: requested ${NUM_SAMPLES} pairs, found ${PAIR_COUNT}" >&2; exit 2; }
...
if [[ -f "${PROJECT_DIR}/scripts/train/run_connected_subword_synthetic.sh" ]]; then NUM_GPUS="${NUM_GPUS:-2}"; printf '%s\n' "Augmented Arabic synthetic training" "  branch=${BRANCH_NAME}" "  dataset=${DATA_DIR}" "  pairs=${PAIR_COUNT}" "  samples=${NUM_SAMPLES}" "  epochs=${EPOCHS}" "  online augmentation=disabled" "  NCCL P2P=disabled" "  GPUs=${NUM_GPUS}" "  Slurm tasks=1" "  job id=${JOB_ID}"; exec bash "${PROJECT_DIR}/scripts/train/run_connected_subword_synthetic.sh"; fi
...
GPU_RESOURCE="${GPU_RESOURCE:-rtx_4090}"; PARTITION="${PARTITION:-rtx4090}"; ACCOUNT="${ACCOUNT:-jelsana}"; QOS="${QOS:-normal}"; CPUS_PER_TASK="${CPUS_PER_TASK:-$((8 * NUM_GPUS))}"; MEMORY="${MEMORY:-96G}"; TIME_LIMIT="${TIME_LIMIT:-2-00:00:00}"; MAIL_USER="${MAIL_USER:-ahmedmas@post.bgu.ac.il}"; SLURM_JOB_NAME="${SLURM_JOB_NAME:-${JOB_ID}}"
printf '%s\n' "Augmented Arabic synthetic training" "  branch=${BRANCH_NAME}" "  dataset=${DATA_DIR}" "  pairs=${PAIR_COUNT}" "  samples=${NUM_SAMPLES}" "  epochs=${EPOCHS}" "  online augmentation=disabled" "  NCCL P2P=disabled" "  Slurm request=${NUM_GPUS} ${GPU_RESOURCE} GPU(s), 1 task, ${CPUS_PER_TASK} CPUs, ${MEMORY}" "  job id=${JOB_ID}"
if [[ -z "${SLURM_JOB_ID:-}" ]]; then sbatch --job-name="${SLURM_JOB_NAME}" --output="${PROJECT_DIR}/out/%x_%J.out" --error="${PROJECT_DIR}/out/%x_%J.err" --chdir="${PROJECT_DIR}" --partition="${PARTITION}" --account="${ACCOUNT}" --qos="${QOS}" --gpus="${GPU_RESOURCE}:${NUM_GPUS}" --ntasks=1 --cpus-per-task="${CPUS_PER_TASK}" --mem="${MEMORY}" --time="${TIME_LIMIT}" --mail-type=ALL --mail-user="${MAIL_USER}" --export=ALL,PROJECT_DIR="${PROJECT_DIR}",DATA_DIR="${DATA_DIR}",JOB_ID="${JOB_ID}",NUM_SAMPLES="${NUM_SAMPLES}",EPOCHS="${EPOCHS}",NUM_GPUS="${NUM_GPUS}",TRAINING_MODE="${TRAINING_MODE}",SYNTHETIC_MANUSCRIPT_AUGMENT=0,REAL_AUGMENT=0,DATASET_TYPE=synthetic,NCCL_P2P_DISABLE=1,NCCL_SHM_DISABLE=0,NCCL_ASYNC_ERROR_HANDLING=1 "${SCRIPT_PATH}"; exit 0; fi
...
if [[ "${TRAINING_MODE}" == generic ]]; then TRAIN_ARGS=(train.py --job_id "${JOB_ID}" --dataset_type synthetic --data_dir "${DATA_DIR}" --num_samples "${NUM_SAMPLES}" --epochs "${EPOCHS}" --no-augment); if (( NUM_GPUS > 1 )); then exec torchrun --standalone --nnodes=1 --nproc_per_node="${NUM_GPUS}" --max_restarts=0 "${TRAIN_ARGS[@]}"; fi; exec python "${TRAIN_ARGS[@]}"; fi
