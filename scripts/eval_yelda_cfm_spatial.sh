#!/usr/bin/env bash
# Foreground image-only evaluation for the CFM-inspired spatial architecture.
set -euo pipefail

PROJECT_DIR="${PROJECT_DIR:-$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)}"
PYTHON_BIN="${PYTHON_BIN:-python}"
WEIGHTS="${WEIGHTS:-${PROJECT_DIR}/Weights/vit_cfm_spatial/model_best.pth}"
...
RESULTS_DIR="${RESULTS_DIR:-${PROJECT_DIR}/Results/Evaluation/Yelda/vit_cfm_spatial/${EVAL_SPLIT}/${REPRESENTATION}_${IMAGE_PREPROCESSING}_$(date +%Y%m%d_%H%M%S)}"
