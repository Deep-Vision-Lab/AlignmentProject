...
def span(path):
    ...
    cols=np.where((arr>128).sum(axis=0)>=2)[0]; return 0.0 if cols.size==0 else float(cols[-1]-cols[0]+1)/float(arr.shape[1])
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--data-dir',required=True); ap.add_argument('--min-recorded-fill',type=float,default=.90); ap.add_argument('--min-pixel-span',type=float,default=.84); ap.add_argument('--expected-negatives',type=int,default=8); a=ap.parse_args(); root=Path(a.data_dir).expanduser().resolve()
    ...
    if m.get('human_category_folders') is not True: raise RuntimeError('Root category folders missing')
    rows=[json.loads(x) for x in (root/'dataset_manifest.jsonl').read_text(encoding='utf-8').splitlines() if x.strip()]; groups={}; mn=1.0
    for r in rows:
        aid=str((r.get('bridge') or {}).get('anchor_id')); groups.setdefault(aid,[]).append(r); s=span(resolve(root,r['B']['line_image_path'])); mn=min(mn,s)
