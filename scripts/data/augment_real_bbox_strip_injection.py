...
def _load_line(dataset_root: Path, pair_id: str, side_name: str, side: dict, target_height: int) -> SourceLine:
...
def _build_donor_index(lines: Iterable[SourceLine], max_run_boxes: int, min_chars: int, max_chars: int) -> dict[tuple[int, str], list[DonorRun]]:
        ...
        for size in range(1, min(max_run_boxes, len(boxes)) + 1):
            for start in range(0, len(boxes) - size + 1):
                ...
                if not canonical or not (min_chars <= compact_length <= max_chars):
                    continue
                x0, x1 = _x_bounds(selected)
                index[(size, canonical)].append(DonorRun(line, start, size, text, canonical, x0, x1))
...
def _copy_source(line: SourceLine) -> LineState:
    return LineState(line.image.copy(), [replace(box) for box in line.boxes], [], [])


def _target_candidates(state: LineState, size: int, donor_width: int, donor_text: str, width_ratio_min: float, width_ratio_max: float) -> list[int]:
    candidates = []
    for start in range(0, len(state.boxes) - size + 1):
        ...
        if not _run_is_x_isolated(state.boxes, start, size):
            continue
        x0, x1 = _x_bounds(selected)
        ratio = float(donor_width) / float(max(1, x1 - x0))
...
def _shift_existing_strips(strips: list[tuple[int, int]], target_x0: int, target_x1: int, delta: int) -> list[tuple[int, int]] | None:
    updated = []
    for x0, x1 in strips:
        if x1 <= target_x0:
...
def _splice_full_height_run(state: LineState, donor: DonorRun, target_start: int, target_height: int) -> LineState | None:
    size = donor.size
    ...
    if new_width <= 0:
        return None
    shifted_strips = _shift_existing_strips(state.injected_strips, target_x0, target_x1, delta)
    if shifted_strips is None:
        return None
    canvas = Image.new("RGB", (new_width, target_height), "white")
    canvas.paste(state.image.crop((0, 0, target_x0, target_height)), (0, 0))
    canvas.paste(donor_strip, (target_x0, 0))
    canvas.paste(state.image.crop((target_x1, 0, state.image.width, target_height)), (target_x0 + donor_width, 0))
    donor_selected = donor.line.boxes[donor.start : donor.start + donor.size]
    shift = float(target_x0 - donor_x0)
    donor_boxes = [replace(box, x0=box.x0 + shift, x1=box.x1 + shift, injected=True) for box in donor_selected]
    before = []
    after = []
    for index, box in enumerate(state.boxes):
        if target_start <= index < target_start + size:
            continue
        if not (box.x1 <= target_x0 + 0.5 or box.x0 >= target_x1 - 0.5):
            return None
        adjusted = _shift_box_x(box, delta) if box.x0 >= target_x1 - 0.5 else box
    ...
    return LineState(canvas, new_boxes, shifted_strips + [inserted_strip], state.operations + [operation])


def _choose_aligned_donors(rng: random.Random, donor_index: dict[tuple[int, str], list[DonorRun]], forbidden_images: set[Path], used_texts: set[str]) -> tuple[DonorRun, DonorRun] | None:
    ...
    for key in keys:
        ...
        first = eligible[0]
        second = next((run for run in eligible[1:] if run.line.image_path != first.line.image_path), None)
        if second is not None:
            return first, second
    return None


def _augment_aligned_pair(rng: random.Random, source_a: SourceLine, source_b: SourceLine, donor_index: dict[tuple[int, str], list[DonorRun]], regions: int, args) -> tuple[LineState, LineState, dict] | None:
    ...
    for region_index in range(regions):
        ...
        for _ in range(args.max_attempts_per_region):
            donors = _choose_aligned_donors(rng, donor_index, forbidden_images, used_texts)
            if donors is None:
                break
            donor_a, donor_b = donors
            candidates_a = _target_candidates(line_a, donor_a.size, donor_a.x1 - donor_a.x0, donor_a.text, args.injection_width_ratio_min, args.injection_width_ratio_max)
            candidates_b = _target_candidates(line_b, donor_b.size, donor_b.x1 - donor_b.x0, donor_b.text, args.injection_width_ratio_min, args.injection_width_ratio_max)
            if not candidates_a or not candidates_b:
                used_texts.add(donor_a.canonical_text)
                continue
            rng.shuffle(candidates_a)
            rng.shuffle(candidates_b)
            new_a = _splice_full_height_run(line_a, donor_a, candidates_a[0], args.height)
            new_b = _splice_full_height_run(line_b, donor_b, candidates_b[0], args.height)
            ...
            region_meta.append({"region": region_index + 1, "shared_text": donor_a.canonical_text, "donor_A": str(donor_a.line.image_path), "donor_B": str(donor_b.line.image_path), "A_operation": line_a.operations[-1], "B_operation": line_b.operations[-1]})
            placed = True
            break
        if not placed:
            return None
...
def _bbox_payload(image_name: str, state: LineState) -> dict:
    return {"image": image_name, "height": state.image.height, "width": state.image.width, "reading_order": "rtl", "boxes": [{"text": box.text, "x0": round(float(box.x0), 3), "y0": round(float(box.y0), 3), "x1": round(float(box.x1), 3), "y1": round(float(box.y1), 3), "injected": bool(box.injected), "origin_image": box.origin_image} for box in state.boxes]}
...
def _save_side(root: Path, name: str, state: LineState) -> dict:
    ...
    bbox_path.write_text(json.dumps(_bbox_payload(image_path.name, state), ensure_ascii=False, indent=2), encoding="utf-8")
    return {"line_image_path": str(image_path), "text_original_path": str(text_path), "bbox_path": str(bbox_path)}
...
def _save_preview(path: Path, source_a: SourceLine, source_b: SourceLine, augmented_a: LineState, augmented_b: LineState, metadata: dict) -> None:
    panel_w, panel_h, header_h = 1100, 190, 34
...
def main() -> None:
    args = parse_args()
    if args.height <= 0 or args.num_pairs <= 0:
        raise ValueError("--height and --num-pairs must be positive")
    ...
    manifest_path = args.manifest.expanduser().resolve() if args.manifest is not None else dataset_root / "dataset_manifest.jsonl"
    ...
    raw_records = [record for record in _read_manifest(manifest_path) if not labels or str(record.get("label_type", "")) in labels]
    if not raw_records:
        ...
        try:
            ...
            for side_name in ("A", "B"):
                ...
                if image_path not in line_cache:
                    line_cache[image_path] = _load_line(dataset_root, pair_id, side_name, side, args.height)
                loaded.append(line_cache[image_path])
            usable_pairs.append((record, loaded[0], loaded[1]))
        except Exception as exc:
            rejected[f"{type(exc).__name__}: {exc}"] += 1
    if not usable_pairs:
        details = "\n".join(f"  {count} x {reason}" for reason, count in rejected.most_common(8))
        raise RuntimeError("No bbox-valid real line pairs are available. This augmentation requires true line-level text-bearing subword boxes; it never estimates bbox positions.\n" + details)
    donor_index = _build_donor_index(line_cache.values(), args.max_run_boxes, args.injection_min_chars, args.injection_max_chars)
    if not donor_index:
        raise RuntimeError("No repeated bbox-exact subword runs were found in at least two distinct real lines.")
    ...
    with output_manifest.open("w", encoding="utf-8") as handle:
        while generated < args.num_pairs:
            ...
            for _ in range(args.max_attempts_per_output):
                ...
                try:
                    result = _augment_aligned_pair(rng, selected_a, selected_b, donor_index, requested_regions, args)
                except Exception as exc:
                ...
                if result is not None:
                    break
            if result is None:
                raise RuntimeError(f"Could not create output {generated + 1}/{args.num_pairs} with {requested_regions} injected region(s) after {args.max_attempts_per_output} attempts")
            ...
            record = {
                ...
                "A": {**saved_a, "line_idx": int(selected_a.line_idx), "source_line_image_path": str(selected_a.image_path), "source_bbox_json": selected_a.annotation_path},
                "B": {**saved_b, "line_idx": int(selected_b.line_idx), "source_line_image_path": str(selected_b.image_path), "source_bbox_json": selected_b.annotation_path},
            }
            handle.write(json.dumps(record, ensure_ascii=False) + "\n")
            if args.preview:
                _save_preview(preview_root / f"preview_{output_index:03d}_{requested_regions}regions.png", selected_a, selected_b, augmented_a, augmented_b, metadata)
            generated += 1
    ...
    (output_root / "augmentation_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
