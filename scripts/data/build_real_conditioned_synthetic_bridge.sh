...
REAL_AUG_SEED="${REAL_AUG_SEED:-4242}"; REAL_BLUR_MIN_RADIUS="${REAL_BLUR_MIN_RADIUS:-0.15}"; REAL_BLUR_MAX_RADIUS="${REAL_BLUR_MAX_RADIUS:-1.00}"; REAL_NOISE_MIN_SIGMA="${REAL_NOISE_MIN_SIGMA:-2.0}"; REAL_NOISE_MAX_SIGMA="${REAL_NOISE_MAX_SIGMA:-8.0}"
...
python scripts/data/augment_bridge_v3_real_lines.py --data-dir "${OUTPUT_DIR}" --seed "${REAL_AUG_SEED}" --blur-min-radius "${REAL_BLUR_MIN_RADIUS}" --blur-max-radius "${REAL_BLUR_MAX_RADIUS}" --noise-min-sigma "${REAL_NOISE_MIN_SIGMA}" --noise-max-sigma "${REAL_NOISE_MAX_SIGMA}"
...
python scripts/data/validate_bridge_v3_real_augmentation.py --data-dir "${OUTPUT_DIR}"
echo "Bridge V3 dataset ready: ${OUTPUT_DIR}"; echo "Real augmentation: Gaussian blur + Gaussian noise; original preserved"; echo "Human folders: ${OUTPUT_DIR}/{real,positive,negative}/<anchor_id>/"
