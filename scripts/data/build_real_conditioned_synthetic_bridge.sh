...
args=(
  ...
  --negatives-per-anchor "${NEGATIVES_PER_ANCHOR}" --negative-ngram "${NEGATIVE_NGRAM}"
  --min-overlap-word-chars "${MIN_OVERLAP_WORD_CHARS}" --max-shared-islands "${MAX_SHARED_ISLANDS}"
  --sentence-min-words "${SENTENCE_MIN_WORDS}" --sentence-max-words "${SENTENCE_MAX_WORDS}"
  --max-sentence-chars "${MAX_SENTENCE_CHARS}" --max-font-chunk-words "${MAX_FONT_CHUNK_WORDS}"
  --segment-gap-min-px "${SEGMENT_GAP_MIN_PX}" --segment-gap-max-px "${SEGMENT_GAP_MAX_PX}"
  --blur-prob "${BLUR_PROB}" --blur-max-radius "${BLUR_MAX_RADIUS}"
  --noise-prob "${NOISE_PROB}" --noise-sigma-max "${NOISE_SIGMA_MAX}"
  --contrast-min "${CONTRAST_MIN}" --contrast-max "${CONTRAST_MAX}"
  --brightness-min "${BRIGHTNESS_MIN}" --brightness-max "${BRIGHTNESS_MAX}"
  --seed "${SEED}" --max-anchors "${MAX_ANCHORS}"
)
