...
def _page_box(record: object, row: int) -> Box | None:
    if not isinstance(record, dict):
        return None
    # Dataset debug/bboxes.json uses x1,y1,x2,y2. Keep a few aliases for safety.
...
def _load_page_level_line_annotations(image_path: Path, line_width: int) -> LineAnnotations:
    ...
    if requested is None or requested < 1 or requested > len(clusters):
        return LineAnnotations(
            (), str(bbox_path), "line_not_found",
            f"requested line={requested}, clustered_lines={len(clusters)}, expected={expected}",
            method, len(clusters), expected, threshold,
        )
    ...
    line_boxes = [
        Box(box.text, box.x0 * scale_x, 0.0, box.x1 * scale_x, 1.0, box.source_row)
        for box in selected
    ]
    line_boxes = sorted(line_boxes, key=lambda box: (-box.center_x, box.source_row))
    detail = (
        f"page_boxes={len(boxes)} selected={len(line_boxes)} line={requested} "
        f"source_width={source_width:.1f} line_width={line_width} scale_x={scale_x:.6f}"
    )
...
def _convert_external_annotations(annotations, source: str) -> LineAnnotations:
    boxes = tuple(
        Box(str(box.text), float(box.x0), float(box.y0), float(box.x1), float(box.y1), int(getattr(box, "source_row", -1)))
...
def _load_annotations(image_path: Path, width: int, height: int) -> LineAnnotations:
    # First try the existing line-aware parser. The current RealAug10K layout is
    # page-level, so this normally falls through to the exact debug/bboxes path.
...
def _manifest_paths(args: argparse.Namespace, root: Path) -> list[Path]:
    ...
    for value in args.manifest:
        ...
        values.append(path if path.is_absolute() else root / path)
    if not values:
...
def _process_row(row: dict, root: Path, output_root: Path, preview_root: Path, args: argparse.Namespace) -> tuple[dict, dict]:
    ...
    image_a = _resolve_path(root, side_a["line_image_path"])
    image_b = _resolve_path(root, side_b["line_image_path"])
    ...
    with Image.open(image_b) as image:
        width_b, height_b = image.size

    ann_a = _load_annotations(image_a, width_a, height_a)
    ann_b = _load_annotations(image_b, width_b, height_b)
    if not ann_a.boxes:
        raise ValueError(f"No bbox annotations for A ({image_a.name}); status={ann_a.status} detail={ann_a.detail}")
    if not ann_b.boxes:
        raise ValueError(f"No bbox annotations for B ({image_b.name}); status={ann_b.status} detail={ann_b.detail}")

    boxes_a, text_source_a = _fill_missing_box_text(list(ann_a.boxes), text_a)
    boxes_b, text_source_b = _fill_missing_box_text(list(ann_b.boxes), text_b)
    ...
    if not pairs:
        raise ValueError(
            "No order-preserving shared bbox text; "
            f"A={labels_a} B={labels_b}"
        )
    runs = _consecutive_runs(pairs)
    intervals_a = _run_intervals(boxes_a, runs, 0, width_a)
    intervals_b = _run_intervals(boxes_b, runs, 1, width_b)
    if not intervals_a or not intervals_b:
        raise ValueError("matched boxes produced no non-empty intervals")
    ...
    if args.preview:
        _save_image(_preview(image_a, mask_a, image_b, mask_b), preview_root / f"{key}.png", args.overwrite)

    side_a["alignment_mask_path"] = _relative(mask_a_path, root)
    side_b["alignment_mask_path"] = _relative(mask_b_path, root)
    updated["alignment_mask_meta"] = {
        "method": "page_bbox_line_cluster_text_lcs_consecutive_runs",
        "shared_subword_boxes": len(pairs),
        "consecutive_runs": len(runs),
        "A": {
            "box_count": len(boxes_a), "matched_box_count": len({i for i, _ in pairs}),
            "intervals_x": [list(v) for v in intervals_a], "bbox_annotation_path": ann_a.source,
            "bbox_status": ann_a.status, "bbox_detail": ann_a.detail, "bbox_text_source": text_source_a,
            "clustering_method": ann_a.clustering_method, "clustered_lines": ann_a.clustered_lines,
            "expected_lines": ann_a.expected_lines, "line_threshold": ann_a.threshold,
            "mask_size": [width_a, height_a],
        },
        "B": {
            "box_count": len(boxes_b), "matched_box_count": len({j for _, j in pairs}),
            "intervals_x": [list(v) for v in intervals_b], "bbox_annotation_path": ann_b.source,
            "bbox_status": ann_b.status, "bbox_detail": ann_b.detail, "bbox_text_source": text_source_b,
            "clustering_method": ann_b.clustering_method, "clustered_lines": ann_b.clustered_lines,
            "expected_lines": ann_b.expected_lines, "line_threshold": ann_b.threshold,
            "mask_size": [width_b, height_b],
        },
    }
    report = {
        "status": "ok", "pair_id": str(row.get("pair_id", "")), "sample_key": key,
        "image_A": str(image_a), "image_B": str(image_b), "boxes_A": len(boxes_a), "boxes_B": len(boxes_b),
        "matched_boxes": len(pairs), "consecutive_runs": len(runs), "intervals_A": intervals_a, "intervals_B": intervals_b,
        "A_cluster_method": ann_a.clustering_method, "A_clustered_lines": ann_a.clustered_lines, "A_expected_lines": ann_a.expected_lines,
        "B_cluster_method": ann_b.clustering_method, "B_clustered_lines": ann_b.clustered_lines, "B_expected_lines": ann_b.expected_lines,
        "mask_A": str(mask_a_path), "mask_B": str(mask_b_path),
    }
...
def main() -> int:
    ...
    if args.write_manifests and args.limit > 0:
        raise SystemExit("Refusing --write-manifests with --limit; preview first, then run the full manifest.")
    ...
    for manifest in manifests:
        ...
        for row_index, row in enumerate(rows):
            if row_index < max(0, args.start_index) or (args.limit > 0 and processed >= args.limit):
            ...
            try:
                if cache_key in cache:
                    ...
                    updated, report = _process_row(row, root, output_root, preview_root, args)
                    cache[cache_key] = (deepcopy(updated), dict(report))
                ok += 1
                print(
                    f"OK manifest={manifest.name} row={row_index} pair={report['pair_id']} "
                    f"boxes={report['boxes_A']}/{report['boxes_B']} matched={report['matched_boxes']} runs={report['consecutive_runs']} "
                    f"A_lines={report['A_clustered_lines']}/{report['A_expected_lines']} "
                    f"B_lines={report['B_clustered_lines']}/{report['B_expected_lines']} "
                    f"A={report['intervals_A']} B={report['intervals_B']}", flush=True,
                )
            except Exception as exc:
                failed += 1
                updated = deepcopy(row)
                updated["alignment_mask_meta"] = {"method": "page_bbox_line_cluster_text_lcs_consecutive_runs", "status": "error", "error": f"{type(exc).__name__}: {exc}"}
                report = {"status": "error", "pair_id": str(row.get("pair_id", "")), "error": f"{type(exc).__name__}: {exc}"}
                print(f"ERROR manifest={manifest.name} row={row_index} pair={row.get('pair_id','')} {report['error']}", file=sys.stderr, flush=True)
                if args.strict:
                    raise
            report.update({"manifest": manifest.name, "row": row_index})
            reports.append(report); updated_rows.append(updated)
        if args.write_manifests:
            destination = _companion(manifest)
    ...
    report_path.write_text("".join(json.dumps(row, ensure_ascii=False) + "\n" for row in reports), encoding="utf-8")
