#!/usr/bin/env bash
# Idempotent CPU preparation for the canonical RealSyntheticBridge V3 dataset.
set -euo pipefail
...
if [[ -e "${BRIDGE_DATA_DIR}" ]]; then
  echo "Existing bridge dataset is missing/outdated (version=${CURRENT_VERSION}); rebuilding V3."
fi
