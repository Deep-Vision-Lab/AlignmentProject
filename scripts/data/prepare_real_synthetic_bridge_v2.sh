...
DATA_DIR="${REAL_DATA_DIR}" OUTPUT_DIR="${BRIDGE_DATA_DIR}" OVERWRITE=1 MAX_ANCHORS="${MAX_ANCHORS:-0}" NEGATIVES_PER_ANCHOR="${NEGATIVES_PER_ANCHOR:-4}" NEGATIVE_NGRAM="${NEGATIVE_NGRAM:-3}" MIN_OVERLAP_WORD_CHARS="${MIN_OVERLAP_WORD_CHARS:-1}" SEED="${SEED:-42}" bash scripts/data/build_real_conditioned_synthetic_bridge.sh
