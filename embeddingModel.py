...
def _denormalize_imagenet_patches(patches: torch.Tensor) -> torch.Tensor:
    if patches.ndim != 5:
        raise ValueError("Expected patches with shape [B, S, C, H, W], " f"got {tuple(patches.shape)}")
    if patches.shape[2] != 3:
        raise ValueError("Ink estimation expects three RGB channels, " f"got C={patches.shape[2]}")
...
def _patch_background_level(gray: torch.Tensor) -> torch.Tensor:
    ...
    border = torch.cat([gray[..., :border_h, :].flatten(start_dim=2), gray[..., -border_h:, :].flatten(start_dim=2), gray[..., :, :border_w].flatten(start_dim=2), gray[..., :, -border_w:].flatten(start_dim=2)], dim=-1)
    return border.median(dim=-1).values.unsqueeze(-1).unsqueeze(-1)


def window_ink_ratio_from_patches(patches: torch.Tensor, contrast_threshold: float | None = None) -> torch.Tensor:
    """Estimate foreground coverage for every horizontal window."""
    if contrast_threshold is None:
        contrast_threshold = float(os.environ.get("INK_CONTRAST_THRESHOLD", _parameter("ink_contrast_threshold", 0.15)))
...
class LineWindowViT(nn.Module):
    ...
    def __init__(self, *, input_height: int, window_size: int, stride: int, embed_dim: int, num_layers: int, num_heads: int, mlp_dim: int, dropout: float, max_tokens: int, position_base_tokens: int) -> None:
        ...
        num_layers, num_heads, mlp_dim, max_tokens, position_base_tokens = int(num_layers), int(num_heads), int(mlp_dim), int(max_tokens), int(position_base_tokens)
        if input_height <= 0 or window_size <= 0 or stride <= 0: raise ValueError("input_height, window_size, and stride must be positive")
        if num_layers <= 0 or num_heads <= 0 or mlp_dim <= 0: raise ValueError("ViT layers, heads, and MLP dimension must be positive")
        if embed_dim % num_heads != 0: raise ValueError(f"VIT_HEADS={num_heads} must divide VECTOR_SIZE={embed_dim}")
        if max_tokens <= 0: raise ValueError("VIT_MAX_TOKENS must be positive")
        if position_base_tokens <= 0 or position_base_tokens > max_tokens: raise ValueError("VIT_POSITION_BASE_TOKENS must be positive and no larger than " f"VIT_MAX_TOKENS={max_tokens}, got {position_base_tokens}")
        self.input_height, self.window_size, self.stride, self.embed_dim, self.max_tokens, self.position_base_tokens = input_height, window_size, stride, embed_dim, max_tokens, position_base_tokens
    ...
    def _reset_parameters(self):
        nn.init.trunc_normal_(self.position_embedding, std=0.02); nn.init.xavier_uniform_(self.patch_embedding.weight)
        if self.patch_embedding.bias is not None: nn.init.zeros_(self.patch_embedding.bias)
    def _position_tokens(self, count: int):
        count = int(count)
    ...
    def forward(self, image: torch.Tensor, *, use_flip: bool):
        if image.ndim != 4 or image.shape[1] != 3: raise ValueError("ViT input must have shape [B, 3, H, W], " f"got {tuple(image.shape)}")
        if int(image.shape[2]) != self.input_height: raise ValueError(f"ViT expects input height {self.input_height}, got {image.shape[2]}")
        if int(image.shape[3]) < self.window_size: raise ValueError(f"Input width {image.shape[3]} is smaller than window size {self.window_size}")
        tokens = self.patch_embedding(image)
        if tokens.shape[2] != 1: raise RuntimeError("Full-height patch embedding should produce one vertical token row, " f"got shape {tuple(tokens.shape)}")
        tokens = tokens.squeeze(2).transpose(1, 2).contiguous()
        if use_flip: tokens = torch.flip(tokens, dims=[1])
        ...
        return self.encoder(self.input_dropout(contextual)), local_tokens
...
class EmbeddingModel(nn.Module):
    ...
    def __init__(self, window_size: int = 32, stride: int = 16, vector_size: int = 128, device: str | torch.device = "cuda", use_flip: bool = False, input_height: int | None = None, vit_layers: int | None = None, vit_heads: int | None = None, vit_mlp_dim: int | None = None, vit_dropout: float | None = None, vit_max_tokens: int | None = None, vit_position_base_tokens: int | None = None, *, use_bilstm: bool = False, bilstm_layers: int | None = None, bilstm_hidden_dim: int | None = None, use_local_grouping: bool | None = False, local_group_size: int | None = None, **_ignored) -> None:
        super().__init__()
        if bool(use_bilstm): raise ValueError("EmbeddingModel is pure ViT on ViT branches; use_bilstm=True is not supported.")
        if bool(use_local_grouping): raise ValueError("EmbeddingModel is pure ViT on ViT branches; use_local_grouping=True is not supported.")
        del bilstm_layers, bilstm_hidden_dim, local_group_size
        self.device, self.window_size, self.stride, self.vector_size, self.use_bilstm = device, int(window_size), int(stride), int(vector_size), False
        self.input_height = int(_parameter("vit_input_height", 128) if input_height is None else input_height)
        self.vit_layers = int(_parameter("vit_layers", _env_int("VIT_LAYERS", 4)) if vit_layers is None else vit_layers)
        self.vit_heads = int(_parameter("vit_heads", _env_int("VIT_HEADS", 4)) if vit_heads is None else vit_heads)
        self.vit_mlp_dim = int(_parameter("vit_mlp_dim", _env_int("VIT_MLP_DIM", 512)) if vit_mlp_dim is None else vit_mlp_dim)
        self.vit_dropout = float(_parameter("vit_dropout", _env_float("VIT_DROPOUT", 0.10)) if vit_dropout is None else vit_dropout)
        self.vit_max_tokens = int(_parameter("vit_max_tokens", _env_int("VIT_MAX_TOKENS", 256)) if vit_max_tokens is None else vit_max_tokens)
        self.vit_position_base_tokens = int(_parameter("vit_position_base_tokens", _env_int("VIT_POSITION_BASE_TOKENS", 63)) if vit_position_base_tokens is None else vit_position_base_tokens)
        self.register_buffer("_use_flip_state", torch.tensor(1 if use_flip else 0, dtype=torch.uint8)); self.register_buffer("_use_local_grouping_state", torch.tensor(0, dtype=torch.uint8))
    ...
    def forward(self, image, show_dims=False, return_local=False, return_ink=False, return_grouped=False):
        contextual, local = self.vit_encoder(image, use_flip=self.use_flip); ink_ratio = None
        if return_ink:
            patches = sliding_window(image, self.window_size, self.stride)
            if self.use_flip: patches = torch.flip(patches, dims=[1])
            ink_ratio = window_ink_ratio_from_patches(patches)
            if ink_ratio.shape[1] != local.shape[1]: raise RuntimeError("ViT token count and ink-window count differ: " f"{local.shape[1]} != {ink_ratio.shape[1]}")
        contextual, local = self.vision_norm(contextual), self.vision_norm(local)
        if show_dims: print("image embeddings: " f"encoder=vit contextual={tuple(contextual.shape)} local={tuple(local.shape)} flip={self.use_flip} position_base_tokens={self.vit_position_base_tokens}", flush=True)
        outputs = [contextual]
...
def build_vit_from_environment(*, window_size, stride, vector_size, device, use_flip):
    return EmbeddingModel(window_size=window_size, stride=stride, vector_size=vector_size, device=device, use_flip=use_flip, input_height=int(_parameter("vit_input_height", _env_int("VIT_INPUT_HEIGHT", 128))), vit_layers=int(_parameter("vit_layers", _env_int("VIT_LAYERS", 4))), vit_heads=int(_parameter("vit_heads", _env_int("VIT_HEADS", 4))), vit_mlp_dim=int(_parameter("vit_mlp_dim", _env_int("VIT_MLP_DIM", 512))), vit_dropout=float(_parameter("vit_dropout", _env_float("VIT_DROPOUT", 0.10))), vit_max_tokens=int(_parameter("vit_max_tokens", _env_int("VIT_MAX_TOKENS", 256))), vit_position_base_tokens=int(_parameter("vit_position_base_tokens", _env_int("VIT_POSITION_BASE_TOKENS", 63))))
def prepare_vit_model(model: EmbeddingModel):
    ...
    if _env_flag("TORCH_COMPILE_VISUAL", False):
        if not hasattr(torch, "compile"): print("torch.compile unavailable; continuing without it", flush=True)
        else:
