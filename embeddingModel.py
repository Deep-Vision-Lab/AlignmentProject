...
def sliding_window(image,window_size,stride):
    return image.unfold(3,int(window_size),int(stride)).permute(0,3,1,2,4).contiguous()
def _denormalize_imagenet_patches(patches):
    if patches.ndim!=5: raise ValueError(f"Expected patches with shape [B, S, C, H, W], got {tuple(patches.shape)}")
    if patches.shape[2]!=3: raise ValueError(f"Ink estimation expects three RGB channels, got C={patches.shape[2]}")
    mean=patches.new_tensor(_IMAGENET_MEAN).view(1,1,3,1,1); std=patches.new_tensor(_IMAGENET_STD).view(1,1,3,1,1)
    return (patches.float()*std+mean).clamp(0.0,1.0)
def _patch_background_level(gray):
    h,w=int(gray.shape[-2]),int(gray.shape[-1]); bh=max(1,int(round(h*.05))); bw=max(1,int(round(w*.05)))
    border=torch.cat([gray[...,:bh,:].flatten(2),gray[...,-bh:,:].flatten(2),gray[...,:,:bw].flatten(2),gray[...,:,-bw:].flatten(2)],dim=-1)
...
def window_ink_ratio_from_patches(patches,contrast_threshold=None):
    if contrast_threshold is None: contrast_threshold=float(os.environ.get("INK_CONTRAST_THRESHOLD","0.15"))
    contrast_threshold=max(0.0,min(1.0,float(contrast_threshold)))
    with torch.no_grad():
        rgb=_denormalize_imagenet_patches(patches.detach()); gray=.2989*rgb[:,:,0]+.5870*rgb[:,:,1]+.1140*rgb[:,:,2]
        ink=(gray-_patch_background_level(gray)).abs().ge(contrast_threshold).float(); return ink.mean(dim=(2,3))
class LineWindowViT(nn.Module):
    def __init__(self,*,input_height,window_size,stride,embed_dim,num_layers,num_heads,mlp_dim,dropout,max_tokens,position_base_tokens):
        ...
        self.input_height=input_height; self.window_size=window_size; self.stride=stride; self.embed_dim=embed_dim; self.max_tokens=max_tokens; self.position_base_tokens=position_base_tokens
        self.patch_embedding=nn.Conv2d(3,embed_dim,kernel_size=(input_height,window_size),stride=(input_height,stride),bias=True); self.local_norm=nn.LayerNorm(embed_dim); self.position_embedding=nn.Parameter(torch.zeros(1,max_tokens,embed_dim)); self.input_dropout=nn.Dropout(float(dropout))
    ...
    def forward(self,image,*,use_flip):
        if image.ndim!=4 or image.shape[1]!=3: raise ValueError(f"ViT input must have shape [B, 3, H, W], got {tuple(image.shape)}")
        ...
        if tokens.shape[2]!=1: raise RuntimeError(f"Full-height patch embedding produced shape {tuple(tokens.shape)}")
        tokens=tokens.squeeze(2).transpose(1,2).contiguous(); tokens=torch.flip(tokens,dims=[1]) if use_flip else tokens; local=self.local_norm(tokens); contextual=local+self._position_tokens(local.shape[1]).to(dtype=local.dtype,device=local.device); return self.encoder(self.input_dropout(contextual)),local
class EmbeddingModel(nn.Module):
    ...
    def __init__(self,window_size=32,stride=16,vector_size=128,device="cuda",use_flip=False,input_height=None,vit_layers=None,vit_heads=None,vit_mlp_dim=None,vit_dropout=None,vit_max_tokens=None,vit_position_base_tokens=None,*,use_bilstm=False,bilstm_layers=None,bilstm_hidden_dim=None,use_local_grouping=False,local_group_size=None,**_ignored):
        ...
        self.device=device; self.window_size=int(window_size); self.stride=int(stride); self.vector_size=int(vector_size); self.input_height=int(_env_int("VIT_INPUT_HEIGHT",128) if input_height is None else input_height); self.use_bilstm=False
        self.vit_layers=int(_env_int("VIT_LAYERS",4) if vit_layers is None else vit_layers); self.vit_heads=int(_env_int("VIT_HEADS",4) if vit_heads is None else vit_heads); self.vit_mlp_dim=int(_env_int("VIT_MLP_DIM",512) if vit_mlp_dim is None else vit_mlp_dim); self.vit_dropout=float(_env_float("VIT_DROPOUT",.1) if vit_dropout is None else vit_dropout); self.vit_max_tokens=int(_env_int("VIT_MAX_TOKENS",256) if vit_max_tokens is None else vit_max_tokens); self.vit_position_base_tokens=int(_env_int("VIT_POSITION_BASE_TOKENS",63) if vit_position_base_tokens is None else vit_position_base_tokens)
    ...
    def forward(self,image,show_dims=False,return_local=False,return_ink=False,return_grouped=False):
        ...
        if return_ink:
            patches=sliding_window(image,self.window_size,self.stride); patches=torch.flip(patches,dims=[1]) if self.use_flip else patches; ink=window_ink_ratio_from_patches(patches)
            if ink.shape[1]!=local.shape[1]: raise RuntimeError(f"ViT token count and ink-window count differ: {local.shape[1]} != {ink.shape[1]}")
...
def build_vit_from_environment(*,window_size,stride,vector_size,device,use_flip): return EmbeddingModel(window_size=window_size,stride=stride,vector_size=vector_size,device=device,use_flip=use_flip,input_height=_env_int("VIT_INPUT_HEIGHT",128),vit_layers=_env_int("VIT_LAYERS",4),vit_heads=_env_int("VIT_HEADS",4),vit_mlp_dim=_env_int("VIT_MLP_DIM",512),vit_dropout=_env_float("VIT_DROPOUT",.1),vit_max_tokens=_env_int("VIT_MAX_TOKENS",256),vit_position_base_tokens=_env_int("VIT_POSITION_BASE_TOKENS",63))
