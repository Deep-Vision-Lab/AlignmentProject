...
for passing train-only positive and no-shared indices after leakage exclusion;
validation and test examples are never synthesized.
"""
...
from RealDataAugmentation import (
    _strip_text_boundaries,
    _with_text_boundaries,
    stitch_rtl_lines,
)
...
def _stitch_many(
    images: list[Image.Image], gap_min: float, gap_max: float
) -> Image.Image:
    if not images:
...
class PartialOverlapRealPairDataset(Dataset):
    """Generate train-only real-handwriting pairs with partial shared content."""
    ...
    def __init__(
        ...
        self.max_shared_islands = max(
            1, min(3, _env_int("REAL_PARTIAL_OVERLAP_MAX_SHARED_ISLANDS", 3))
        )
        self.max_text_chars = max(
            32, _env_int("REAL_PARTIAL_OVERLAP_MAX_TEXT_CHARS", 150)
        )
        self.multi_island_probability = max(
            0.0,
            min(
                1.0,
                _env_float("REAL_PARTIAL_OVERLAP_MULTI_ISLAND_PROB", 0.65),
            ),
        )
        self.three_island_probability = max(
            ...
            min(
                1.0,
                _env_float("REAL_PARTIAL_OVERLAP_THREE_ISLAND_PROB", 0.15),
            ),
        )
        self.edge_distractor_probability = max(
            ...
            min(
                1.0,
                _env_float("REAL_PARTIAL_OVERLAP_EDGE_DISTRACTOR_PROB", 0.45),
            ),
        )
        self.gap_min = max(
            0.0, _env_float("REAL_AUG_STITCH_GAP_MIN", 0.05)
        )
        self.gap_max = max(
            self.gap_min, _env_float("REAL_AUG_STITCH_GAP_MAX", 0.12)
        )
        self.max_attempts = max(
            4, _env_int("REAL_PARTIAL_OVERLAP_MAX_ATTEMPTS", 24)
        )

        # Prefer short distractors so multiple islands are not excessively
        # compressed when the final composite is resized to the fixed canvas.
        distractor_lengths = []
        ...
        self.short_distractor_indices = [
            index for _length, index in distractor_lengths[:keep]
        ]
    ...
    def _shared_count(self) -> int:
        if (
            self.max_shared_islands <= 1
            or random.random() > self.multi_island_probability
        ):
            return 1
        if (
            self.max_shared_islands >= 3
            and random.random() < self.three_island_probability
        ):
            return 3
        return 2
    ...
    def _choose_shared(self, anchor: int, count: int) -> list[int]:
        ...
        if need:
            if len(pool) >= need:
                ...
                chosen.extend(
                    random.choice(pool or [anchor]) for _ in range(need)
                )
    ...
    def _build_once(self, anchor: int, shared_count: int):
        ...
        for shared_position, index in enumerate(shared_indices):
            ...
            segments_a.append(
                (
                    _read_raw_image(self.positive_dataset, sample["A"]),
                    text_a,
                    "shared",
                )
            )
            segments_b.append(
                (
                    _read_raw_image(self.positive_dataset, sample["B"]),
                    text_b,
                    "shared",
                )
            )
            ...
            if shared_position + 1 < len(shared_indices):
                # Mandatory unmatched content separates consecutive shared islands.
                add_distractor()
        ...
        text_a = " ".join(
            text for _image, text, _kind in segments_a if text
        ).strip()
        text_b = " ".join(
            text for _image, text, _kind in segments_b if text
        ).strip()
        ...
        image_a = _stitch_many(
            [image for image, _text, _kind in segments_a],
            self.gap_min,
            self.gap_max,
        )
        image_b = _stitch_many(
            [image for image, _text, _kind in segments_b],
            self.gap_min,
            self.gap_max,
        )
        ...
        shared_islands = sum(
            1 for _image, _text, kind in segments_a if kind == "shared"
        )
        distractor_islands = sum(
            1 for _image, _text, kind in segments_a if kind == "distractor"
        )
        return {
            ...
            "image2": image_b,
            # The existing positive pair loss matches equal transcript spans.
            # medium_match therefore activates only genuinely shared text spans.
    ...
    def __getitem__(self, index):
        ...
        for attempt in range(self.max_attempts):
            # Gracefully back off 3 -> 2 -> 1 if long composites exceed the
            # configured transcript length cap.
            shared_count = max(
                1,
                requested
                - (attempt // max(1, self.max_attempts // 3)),
            )
        ...
        try:
            ...
            if result is None:
                raise RuntimeError(
                    "Could not construct a feasible partial-overlap sample"
                )
            return result
        finally:
