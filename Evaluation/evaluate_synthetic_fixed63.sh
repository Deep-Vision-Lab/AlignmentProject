#!/usr/bin/env bash
# Evaluate the stroke-aware fixed-63 model on the exact held-out synthetic test split.
set -euo pipefail
set -a
...
RESULTS_ROOT="${RESULTS_ROOT:-${PROJECT_DIR}/Results/Evaluation/stroke_aware_subword_cnn/Synthetic_Experiments/${RUN_ID}}"
...
EVAL_JOB_NAME="${EVAL_JOB_NAME:-eval_stroke_syn63}"
...
print_config() {
  printf '%s\n' \
    "Stroke-aware fixed-63 synthetic evaluation" \
