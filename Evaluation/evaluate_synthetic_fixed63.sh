#!/usr/bin/env bash
# Evaluate the training-speed CNN+BiLSTM checkpoint on the exact held-out fixed-63 synthetic test split.
set -euo pipefail
set -a
...
RESULTS_ROOT="${RESULTS_ROOT:-${PROJECT_DIR}/Results/Evaluation/training_speed_optimization/Synthetic_Experiments/${RUN_ID}/SmithWaterman}"
...
EVAL_JOB_NAME="${EVAL_JOB_NAME:-eval_trainopt_sw63}"
...
print_config() {
  printf '%s\n' \
    "Training-speed fixed-63 synthetic Smith-Waterman evaluation" \
...
printf '%s\n' \
  "Synthetic Smith-Waterman evaluation finished." \
