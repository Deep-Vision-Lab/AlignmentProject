...
def _fill_missing_text(boxes: list[SubwordBox], image_path: Path) -> list[SubwordBox]:
    ...
    if len(tokens) != len(boxes):
        return boxes
    return [SubwordBox(token, box.x0, box.y0, box.x1, box.y1, box.source_row) for token, box in zip(tokens, boxes)]
...
def _binary_metrics(tp: int, fp: int, fn: int, tn: int) -> dict[str, float]:
    ...
    return {"precision": precision, "recall": recall, "f1": f1, "specificity": _ratio(tn, tn + fp, 1.0), "accuracy": _ratio(tp + tn, tp + fp + fn + tn, 0.0)}


def _box_coverage(box: SubwordBox, interval) -> float:
    if interval is None or box.width <= 0:
        return 0.0
    return max(0.0, min(box.x1, interval[1]) - max(box.x0, interval[0])) / box.width
...
def _predicted_indices(boxes, interval) -> tuple[set[int], list[float]]:
    ...
    for index, box in enumerate(boxes):
        coverage = _box_coverage(box, interval); coverages.append(coverage)
...
def _union_interval(boxes, indices: set[int]):
    return None if not indices else (min(boxes[index].x0 for index in indices), max(boxes[index].x1 for index in indices))


def _interval_iou(left, right):
    if left is None or right is None:
        return None
    intersection = max(0.0, min(left[1], right[1]) - max(left[0], right[0])); union = max(left[1], right[1]) - min(left[0], right[0])
...
def _raster_metrics(boxes, gt_indices: set[int], interval, width: int, height: int):
    ...
    for index in gt_indices:
        box = boxes[index]; x0, x1 = int(math.floor(box.x0)), int(math.ceil(box.x1)); y0, y1 = int(math.floor(box.y0)), int(math.ceil(box.y1))
        gt[max(0, y0):min(height, y1), max(0, x0):min(width, x1)] = True
    pred = np.zeros_like(gt)
    if interval is not None:
        x0, x1 = int(math.floor(interval[0])), int(math.ceil(interval[1])); pred[:, max(0, x0):min(width, x1)] = True
    intersection = int(np.logical_and(gt, pred).sum()); pred_area, gt_area = int(pred.sum()), int(gt.sum()); union = int(np.logical_or(gt, pred).sum())
    precision = _ratio(intersection, pred_area, 1.0 if gt_area == 0 else 0.0); recall = _ratio(intersection, gt_area, 1.0)
    return {"precision": precision, "recall": recall, "f1": _ratio(2 * precision * recall, precision + recall, 0.0), "iou": _ratio(intersection, union, 1.0)}


def _predicted_interval(row: dict, prefix: str, width: int, use_flip: bool):
    try:
        start = int(row[f"{prefix}_path_start"]); end = int(row[f"{prefix}_path_end"]); windows = int(row[f"{prefix}_windows"])
...
def _line_metrics(prefix: str, annotations: BoxAnnotations, gt_indices: set[int], interval, width: int, height: int):
    boxes = annotations.boxes; predicted, coverages = _predicted_indices(boxes, interval)
    tp = len(predicted & gt_indices); fp = len(predicted - gt_indices); fn = len(gt_indices - predicted); tn = len(set(range(len(boxes))) - predicted - gt_indices)
    binary = _binary_metrics(tp, fp, fn, tn); gt_interval = _union_interval(boxes, gt_indices); interval_iou = _interval_iou(interval, gt_interval); raster = _raster_metrics(boxes, gt_indices, interval, width, height)
    return {
        f"{prefix}_box_annotation_path": annotations.workbook, f"{prefix}_box_annotation_sheet": annotations.sheet, f"{prefix}_box_annotation_status": annotations.status, f"{prefix}_box_annotation_error": annotations.error,
        f"{prefix}_box_count": len(boxes), f"{prefix}_shared_gt_boxes": len(gt_indices), f"{prefix}_predicted_mask_boxes": len(predicted), f"{prefix}_box_tp": tp, f"{prefix}_box_fp": fp, f"{prefix}_box_fn": fn, f"{prefix}_box_tn": tn,
        f"{prefix}_box_precision": binary["precision"], f"{prefix}_box_recall": binary["recall"], f"{prefix}_box_f1": binary["f1"], f"{prefix}_box_specificity": binary["specificity"], f"{prefix}_box_accuracy": binary["accuracy"],
        f"{prefix}_mean_box_mask_coverage": float(np.mean(coverages)) if coverages else None, f"{prefix}_shared_box_mask_coverage": float(np.mean([coverages[index] for index in gt_indices])) if gt_indices else None,
        f"{prefix}_box_interval_iou": interval_iou, f"{prefix}_box_pixel_precision": raster["precision"], f"{prefix}_box_pixel_recall": raster["recall"], f"{prefix}_box_pixel_f1": raster["f1"], f"{prefix}_box_pixel_iou": raster["iou"],
        f"{prefix}_pred_start_px": None if interval is None else float(interval[0]), f"{prefix}_pred_end_px": None if interval is None else float(interval[1]), f"{prefix}_gt_start_px": None if gt_interval is None else float(gt_interval[0]), f"{prefix}_gt_end_px": None if gt_interval is None else float(gt_interval[1]),
        f"{prefix}_region_iou": interval_iou, f"{prefix}_start_error_px": None if interval is None or gt_interval is None else abs(interval[0] - gt_interval[0]), f"{prefix}_end_error_px": None if interval is None or gt_interval is None else abs(interval[1] - gt_interval[1]),
    }
...
def metrics_from_evaluation_row(row: dict, pair, models) -> dict:
    ...
    with Image.open(pair.image1) as opened: original_width1, original_height1 = opened.size
    with Image.open(pair.image2) as opened: original_width2, original_height2 = opened.size
    model_width = int(os.environ.get("LINE_WIDTH", "1024")); model_height = int(os.environ.get("LINE_HEIGHT", "128"))
    width1 = model_width if row.get("binarized") else original_width1; height1 = model_height if row.get("binarized") else original_height1; width2 = model_width if row.get("binarized") else original_width2; height2 = model_height if row.get("binarized") else original_height2
    ann1 = load_line_annotations(pair.image1, width1, height1); ann2 = load_line_annotations(pair.image2, width2, height2)
    ...
    texts1 = [_normalise_text(box.text) for box in ann1.boxes]; texts2 = [_normalise_text(box.text) for box in ann2.boxes]; matches = _lcs_pairs(texts1, texts2) if ann1.boxes and ann2.boxes else []
    gt1 = {left for left, _ in matches}; gt2 = {right for _, right in matches}; use_flip = bool(models.image_model.use_flip)
    line1 = _line_metrics("line1", ann1, gt1, _predicted_interval(row, "line1", width1, use_flip), width1, height1); line2 = _line_metrics("line2", ann2, gt2, _predicted_interval(row, "line2", width2, use_flip), width2, height2)
    tp = int(line1["line1_box_tp"]) + int(line2["line2_box_tp"]); fp = int(line1["line1_box_fp"]) + int(line2["line2_box_fp"]); fn = int(line1["line1_box_fn"]) + int(line2["line2_box_fn"]); tn = int(line1["line1_box_tn"]) + int(line2["line2_box_tn"]); micro = _binary_metrics(tp, fp, fn, tn)
    interval_ious = [value for value in (line1["line1_region_iou"], line2["line2_region_iou"]) if value is not None]; pixel_ious = [value for value in (line1["line1_box_pixel_iou"], line2["line2_box_pixel_iou"]) if value is not None]
    status = "missing_annotations" if not ann1.boxes or not ann2.boxes else "annotation_warning" if ann1.status != "ok" or ann2.status != "ok" else "no_shared_subword_boxes" if not matches else "ok"
    return {"real_box_evaluated": bool(ann1.boxes and ann2.boxes), "real_box_status": status, "real_box_match_rule": os.environ.get("REAL_BOX_IN_MASK_RULE", "center"), "real_box_min_coverage": _number("REAL_BOX_MIN_COVERAGE", 0.50), "real_box_coordinate_space": os.environ.get("REAL_BOX_COORDINATE_SPACE", "original"), "shared_subword_matches": len(matches), "pair_box_tp": tp, "pair_box_fp": fp, "pair_box_fn": fn, "pair_box_tn": tn, "pair_box_precision": micro["precision"], "pair_box_recall": micro["recall"], "pair_box_f1": micro["f1"], "pair_box_specificity": micro["specificity"], "pair_box_accuracy": micro["accuracy"], "mean_box_interval_iou": float(np.mean(interval_ious)) if interval_ious else None, "mean_box_pixel_iou": float(np.mean(pixel_ious)) if pixel_ious else None, "mean_region_iou": float(np.mean(interval_ious)) if interval_ious else None, **line1, **line2}


def fieldnames() -> list[str]:
    result = ["real_box_evaluated", "real_box_status", "real_box_match_rule", "real_box_min_coverage", "real_box_coordinate_space", "shared_subword_matches", "pair_box_tp", "pair_box_fp", "pair_box_fn", "pair_box_tn", "pair_box_precision", "pair_box_recall", "pair_box_f1", "pair_box_specificity", "pair_box_accuracy", "mean_box_interval_iou", "mean_box_pixel_iou"]
    for prefix in ("line1", "line2"):
        result.extend([f"{prefix}_box_annotation_path", f"{prefix}_box_annotation_sheet", f"{prefix}_box_annotation_status", f"{prefix}_box_annotation_error", f"{prefix}_box_count", f"{prefix}_shared_gt_boxes", f"{prefix}_predicted_mask_boxes", f"{prefix}_box_tp", f"{prefix}_box_fp", f"{prefix}_box_fn", f"{prefix}_box_tn", f"{prefix}_box_precision", f"{prefix}_box_recall", f"{prefix}_box_f1", f"{prefix}_box_specificity", f"{prefix}_box_accuracy", f"{prefix}_mean_box_mask_coverage", f"{prefix}_shared_box_mask_coverage", f"{prefix}_box_interval_iou", f"{prefix}_box_pixel_precision", f"{prefix}_box_pixel_recall", f"{prefix}_box_pixel_f1", f"{prefix}_box_pixel_iou"])
    return result


def aggregate(rows: list[dict]) -> dict:
    annotated = [row for row in rows if row.get("real_box_evaluated") in (True, "True", 1, "1")]
    tp = sum(int(float(row.get("pair_box_tp") or 0)) for row in annotated); fp = sum(int(float(row.get("pair_box_fp") or 0)) for row in annotated); fn = sum(int(float(row.get("pair_box_fn") or 0)) for row in annotated); tn = sum(int(float(row.get("pair_box_tn") or 0)) for row in annotated); micro = _binary_metrics(tp, fp, fn, tn)
    def values(key: str):
        output = []
    ...
    statuses = {}
    for row in rows:
        status = str(row.get("real_box_status") or "not_evaluated"); statuses[status] = statuses.get(status, 0) + 1
    mean = lambda items: float(np.mean(items)) if items else None
    return {"real_box_samples": len(annotated), "real_box_missing_or_unusable_samples": len(rows) - len(annotated), "real_box_status_counts": statuses, "box_micro_tp": tp, "box_micro_fp": fp, "box_micro_fn": fn, "box_micro_tn": tn, "box_micro_precision": micro["precision"], "box_micro_recall": micro["recall"], "box_micro_f1": micro["f1"], "box_micro_specificity": micro["specificity"], "box_micro_accuracy": micro["accuracy"], "box_macro_precision": mean(values("pair_box_precision")), "box_macro_recall": mean(values("pair_box_recall")), "box_macro_f1": mean(values("pair_box_f1")), "mean_box_interval_iou": mean(values("mean_box_interval_iou")), "mean_box_pixel_iou": mean(values("mean_box_pixel_iou")), "mean_shared_subword_matches": mean(values("shared_subword_matches"))}
