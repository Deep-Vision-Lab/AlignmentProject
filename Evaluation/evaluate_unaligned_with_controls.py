...
def _mask_nonempty(root: Path, role: int, index: int) -> bool:
    candidates = [root / "masks" / f"mask{role}_{index}.png", root / f"mask{role}_{index}.png"]
...
def _synthetic_controls(args, count: int) -> list[dict]:
    ...
    for index in indices:
        ...
        controls.append({
            ...
            "source_index1": index, "source_index2": index,
            ...
            "sample_kind": "ALIGNED_CONTROL", "expected_mask": True,
        })
        if len(controls) >= count:
            break
    if len(controls) < count:
        raise RuntimeError(f"Found only {len(controls)} synthetic aligned controls with non-empty GT masks; requested {count}.")
    return controls
...
def _real_controls(args, count: int) -> list[dict]:
    ...
    namespace = SimpleNamespace(
        arabic_manifest=str(manifest), data_dir=str(root), real_text_key=args.real_text_key,
        real_labels="high_match,medium_match", real_min_text_score=0.0,
        real_validate_paths=True, split_seed=args.split_seed, real_split=args.real_split,
    )
    ...
    for pair in pairs[:count]:
        controls.append({
            "image1": Path(pair.image1), "image2": Path(pair.image2),
            "text1": "", "text2": "",
            "source_index1": pair.manifest_position, "source_index2": pair.manifest_position,
            "pair_id": pair.pair_id, "label_type": f"aligned_control_{pair.label_type}",
            "selection_diagnostics": {"source_label": pair.label_type},
            "sample_kind": "ALIGNED_CONTROL", "expected_mask": True,
        })
    if len(controls) < count:
        raise RuntimeError(f"Found only {len(controls)} real aligned controls in split={args.real_split}; requested {count}.")
    return controls


def _negative_records(args) -> list[dict]:
    records = base._select_synthetic_negatives(args) if args.dataset_type == "synthetic" else base._select_real_negatives(args)
...
def _add_control_banner(path: Path, kind: str, expected_mask: bool, detected_mask: bool) -> None:
    ...
    draw.text((14, 27), "Same _evaluate_one() prediction function is used for aligned controls and unaligned negatives.", fill="black")
    canvas.save(path)
...
def main():
    ...
    controls = _synthetic_controls(args, args.n_aligned_controls) if args.dataset_type == "synthetic" else _real_controls(args, args.n_aligned_controls)
    records = negatives + controls
    random.Random(args.seed + 314159).shuffle(records)
    serializable = []
    for record in records:
        item = dict(record); item["image1"] = str(item["image1"]); item["image2"] = str(item["image2"]); serializable.append(item)
    (output_dir / "selected_mixed_pairs.json").write_text(json.dumps(serializable, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    for position, record in enumerate(records, start=1):
        ...
        detected_mask = not bool(row["zero_mask"]); expected_mask = bool(record["expected_mask"])
        row["index"] = position; row["sample_kind"] = kind; row["expected_mask"] = expected_mask
        row["detected_mask"] = detected_mask; row["prediction_matches_expectation"] = detected_mask == expected_mask
        _add_control_banner(output, kind, expected_mask, detected_mask)
        rows.append(row)
        print(f"[{position}/{len(records)}] {kind} pair_id={row['pair_id']} expected_mask={expected_mask} detected_mask={detected_mask} components={row['predicted_components']}", flush=True)
    fieldnames = ["index", "sample_kind", "expected_mask", "detected_mask", "prediction_matches_expectation", "pair_id", "label_type", "source_index1", "source_index2", "algorithm", "score", "normalized_score", "predicted_components", "zero_mask", "false_positive_mask", "false_mask_fraction_line1", "false_mask_fraction_line2", "mean_false_mask_fraction", "image1", "image2", "output"]
