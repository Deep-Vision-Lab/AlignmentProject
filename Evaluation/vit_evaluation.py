...
def install_vit_evaluation_loader() -> None:
    ...
    def load_evaluation_models(weights_path, device="auto", load_text_model=True):
        ...
        encoder_type = str(config.get("visual_encoder_type", os.environ.get("VISUAL_ENCODER_TYPE", "vit"))).strip().lower()
        if encoder_type != "vit":
        ...
        def vit_constructor(window_size=32, stride=16, vector_size=128, device="cpu", use_flip=False, **_ignored):
            return EmbeddingModel(
                window_size=int(window_size), stride=int(stride), vector_size=int(vector_size), device=device,
                use_flip=_bool(use_flip), input_height=int(config.get("vit_input_height", 128)),
                vit_layers=int(config.get("vit_layers", 4)), vit_heads=int(config.get("vit_heads", 4)),
                vit_mlp_dim=int(config.get("vit_mlp_dim", 512)), vit_dropout=float(config.get("vit_dropout", 0.10)),
