#!/usr/bin/env bash
# Evaluate the stroke-aware fixed-63 checkpoint with global Needleman-Wunsch.
set -euo pipefail
set -a
...
RUN_ID="${RUN_ID:-stroke_aware_augmented_fixed63_27k}"
...
RESULTS_ROOT="${RESULTS_ROOT:-${PROJECT_DIR}/Results/Evaluation/stroke_aware_subword_cnn/Synthetic_Experiments/${RUN_ID}/NeedlemanWunsch}"
...
EVAL_JOB_NAME="${EVAL_JOB_NAME:-eval_stroke_nw63}"
...
print_config() {
  printf '%s\n' \
    "Stroke-aware fixed-63 synthetic Needleman-Wunsch evaluation" \
...
for output_index, zero_based in enumerate(selected_zero_based, start=1):
    ...
    required = [
        root / "images" / f"img1_{dataset_index}.png",
        root / "images" / f"img2_{dataset_index}.png",
        root / "texts" / f"text1_{dataset_index}.txt",
        root / "texts" / f"text2_{dataset_index}.txt",
        root / "masks" / f"mask1_{dataset_index}.png",
        root / "masks" / f"mask2_{dataset_index}.png",
    ]
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise SystemExit("Missing synthetic sample files: " + ", ".join(missing))
    image1, image2, text1, text2, _mask1, _mask2 = required
    lengths = (
...
export DIRECT_SUBWORD_STROKE_INPUT=1
export DIRECT_SUBWORD_STROKE_AUGMENT=0
...
print_config
"${EVAL_PYTHON}" -m Evaluation.eval_stroke_aware_synthetic_nw \
  --weights "${WEIGHTS}" \
  --device cuda \
  --data-dir "${DATA_DIR}" \
...
printf '%s\n' \
  "Needleman-Wunsch synthetic evaluation finished." \
