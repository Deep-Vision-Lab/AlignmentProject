...
export RESULTS_ROOT="${RESULTS_ROOT:-${ROOT}/Results/Evaluation/training_speed_optimization/Synthetic_Experiments/${RUN_ID}/NeedlemanWunsch_components_v2}"
export EVAL_JOB_NAME="${EVAL_JOB_NAME:-eval_trainopt_nw63_cmp2}"
...
# Null-alignment guard: a negative global NW score does not by itself reject a
# local match, because the dataset contains unaligned context. It rejects only
# when the surviving component coverage is also very small.
...
printf '%s\n' \
  "Component-aware NW region settings v2" \
  ...
  "  weak-global veto: score<${NW_COMPONENT_WEAK_GLOBAL_SCORE} and coverage<${NW_COMPONENT_WEAK_GLOBAL_MIN_COVERAGE}" \
  "  output=${RESULTS_ROOT}"
