"""CLI runner for global image-to-image Needleman-Wunsch evaluation."""
...
from Evaluation.nw_core import save_visualization, select_heatmap_matrix, trace_alignment
...
def evaluate_sample(
    ...
    try:
        ...
        trace = trace_alignment(result, match_scores, gap)
    ...
    global_cosines = [float(raw_similarity[i, j]) for i, j in trace.global_path]
    region_cosines = [float(raw_similarity[i, j]) for i, j in trace.region_path]
    region_metrics = alignment_region_metrics(
        trace.region_path, trace.region_traceback, raw_similarity.shape
    )
    mask_metrics = synthetic_mask_region_metrics(
        trace.region_path,
        trace.region_traceback,
    ...
    row = {
        ...
        "region_score": float(trace.region_score),
        "global_steps": int(len(result.steps)),
        "global_match_steps": int(len(trace.global_path)),
        "gap_in_line1": int(trace.gap_in_line1),
        "gap_in_line2": int(trace.gap_in_line2),
        **region_metrics,
        "dp_max_row": max_row,
        "dp_max_col": max_col,
        ...
        "mean_path_cosine": float(np.mean(global_cosines)) if global_cosines else 0.0,
        "mean_region_cosine": float(np.mean(region_cosines)) if region_cosines else 0.0,
    ...
    print(
        ...
        f"nw_score={result.score:.6f} normalized={result.normalized_score:.6f} "
        f"global_steps={row['global_steps']} gaps=({trace.gap_in_line1},{trace.gap_in_line2}) "
        f"region_steps={row['path_steps']} mean_cosine={row['mean_path_cosine']:.4f} "
...
def aggregate(rows):
    ...
    return {
        ...
        "failed": len(failed),
        "mean_score": mean(_numeric_values(successful, "score")),
        "mean_normalized_score": mean(_numeric_values(successful, "normalized_score")),
        "mean_region_score": mean(_numeric_values(successful, "region_score")),
        "mean_global_steps": mean(_numeric_values(successful, "global_steps")),
        "mean_global_match_steps": mean(_numeric_values(successful, "global_match_steps")),
        "mean_gap_in_line1": mean(_numeric_values(successful, "gap_in_line1")),
        "mean_gap_in_line2": mean(_numeric_values(successful, "gap_in_line2")),
        "mean_path_cosine": mean(_numeric_values(successful, "mean_path_cosine")),
        "mean_region_cosine": mean(_numeric_values(successful, "mean_region_cosine")),
        ...
        "terminal_dp_maxima": sum(
            bool(row.get("dp_max_is_terminal")) for row in successful
        ),
...
def batch_fieldnames():
    return [
        ...
        "status", "algorithm", "score", "normalized_score", "region_score",
        "global_steps", "global_match_steps", "gap_in_line1", "gap_in_line2",
        "path_steps", "traceback_steps", "warp_steps", "dp_max_row", "dp_max_col",
        "dp_max_is_terminal", "mean_path_cosine", "mean_region_cosine",
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="offset subtracted from the selected score before NW diagonal updates",
    )
    ...
    parser.add_argument("--output", default="Results/Evaluation/NW/needleman_wunsch.png")
...
def main():
    ...
    common = dict(
        ...
        save_binary=not args.no_save_binarized_images,
    ...
    if not args.batch:
        pair = (
            ...
            if args.image1
            else pair_for_index(args, args.index, manifest_pairs)
        )
