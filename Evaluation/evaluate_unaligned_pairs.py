...
def _select_synthetic_negatives(args) -> list[dict]:
    ...
    if len(selected) < args.n_samples:
        raise RuntimeError(
            f"Could select only {len(selected)} strict synthetic negatives; requested {args.n_samples}."
        )
    return selected
...
def _select_real_negatives(args) -> list[dict]:
    ...
    if not pairs:
        raise RuntimeError("No real no_shared_content samples were found in the requested split.")
    return [
        {
...
def _component_intervals(components, axis: int, n_windows: int, width: int, use_flip: bool):
    ...
    for component in components:
        values = [int(pair[axis]) for pair in component.get("path", ())]
        if not values:
            continue
        left, right = patch_range_to_pixels(min(values), max(values) + 1, n_windows, width, use_flip)
...
def _draw_intervals(ax, array, intervals):
    for left, right in intervals:
        ax.add_patch(Rectangle((left, 1), max(2.0, right-left), max(2.0, array.shape[0]-2),
                               facecolor="red", edgecolor="red", alpha=0.30, linewidth=1.5))


def _save_visualization(output, arr1, arr2, similarity, path, accepted_path,
                        intervals1, intervals2, title, no_mask):
    ...
    ax.set_xlabel("line B windows"); ax.set_ylabel("line A windows"); ax.set_title(title)
...
def _evaluate_one(models, record, dataset_type, args, output):
    ...
    raw_similarity = compute_similarity(features1.select(args.feature), features2.select(args.feature)).cpu().numpy()
    ...
    match_scores = ink_aware_match_scores(match_scores,
        features1.ink.detach().cpu().numpy(), features2.ink.detach().cpu().numpy())
    ...
    if dataset_type == "synthetic":
        ...
        global_path = [(int(step.index1), int(step.index2)) for step in result.steps
                       if step.index1 is not None and step.index2 is not None]
        intervals1 = _component_intervals(components, 0, raw_similarity.shape[0], arr1.shape[1], use_flip)
        intervals2 = _component_intervals(components, 1, raw_similarity.shape[1], arr2.shape[1], use_flip)
        predicted_components = len(components)
        score = float(result.score); normalized_score = float(result.normalized_score)
        algorithm = "needleman_wunsch_component_v3_local"
    else:
        path, score, _dp, _traceback = smith_waterman(raw_similarity, threshold=args.threshold,
            gap_penalty=args.gap, return_traceback=True, match_scores=match_scores)
        global_path = list(path); accepted_path = list(path)
        intervals1 = _sw_intervals(path, 0, raw_similarity.shape[0], arr1.shape[1], use_flip)
        intervals2 = _sw_intervals(path, 1, raw_similarity.shape[1], arr2.shape[1], use_flip)
    ...
    coverage1 = _coverage(intervals1, arr1.shape[1]); coverage2 = _coverage(intervals2, arr2.shape[1])
    _save_visualization(output, arr1, arr2, raw_similarity, global_path, accepted_path,
                        intervals1, intervals2,
                        f"unaligned {dataset_type} | {algorithm} | score={score:.4f}", no_mask)
    return {
        "pair_id": record["pair_id"], "label_type": record["label_type"],
        "source_index1": int(record["source_index1"]), "source_index2": int(record["source_index2"]),
        "algorithm": algorithm, "score": float(score), "normalized_score": normalized_score,
        "predicted_components": int(predicted_components), "zero_mask": bool(no_mask),
        "false_positive_mask": bool(not no_mask), "false_mask_fraction_line1": coverage1,
        "false_mask_fraction_line2": coverage2,
        "mean_false_mask_fraction": float(0.5*(coverage1+coverage2)),
        "image1": str(record["image1"]), "image2": str(record["image2"]), "output": str(output),
    }


def _aggregate(rows, dataset_type):
    total = len(rows); zero = sum(bool(row["zero_mask"]) for row in rows); false_positive = total-zero
    mean = lambda key: float(np.mean([float(row[key]) for row in rows])) if rows else 0.0
    return {
        "dataset_type": dataset_type, "samples": total, "zero_mask_pairs": zero,
        "false_positive_pairs": false_positive, "zero_mask_rate": float(zero/total) if total else 0.0,
...
def parse_args():
    ...
    parser.add_argument("--weights", required=True); parser.add_argument("--dataset-type", choices=("synthetic","real"), required=True)
    parser.add_argument("--data-dir", required=True); parser.add_argument("--arabic-manifest", default=None)
    parser.add_argument("--output-dir", required=True); parser.add_argument("--n-samples", type=int, default=20)
    parser.add_argument("--start-index", type=int, default=1); parser.add_argument("--num-samples", type=int, default=27000)
    parser.add_argument("--split-seed", type=int, default=42); parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--real-split", choices=("train","valid","test","all"), default="test")
    parser.add_argument("--real-text-key", default="text_original_path"); parser.add_argument("--device", default="cuda")
    ...
    parser.add_argument("--score-clip", type=float, default=4.0); parser.add_argument("--threshold", type=float, default=0.45)
    parser.add_argument("--gap", type=float, default=-0.30); parser.add_argument("--min-shared-word-chars", type=int, default=4)
...
def main():
    args = parse_args()
    if args.n_samples <= 0: raise SystemExit("--n-samples must be positive")
    ...
    records = _select_synthetic_negatives(args) if args.dataset_type == "synthetic" else _select_real_negatives(args)
    (output_dir / "selected_unaligned_pairs.json").write_text(json.dumps([
        {**{k:v for k,v in record.items() if k not in {"image1","image2"}},
         "image1":str(record["image1"]), "image2":str(record["image2"])} for record in records
    ], ensure_ascii=False, indent=2), encoding="utf-8")
    models = load_evaluation_models(args.weights, args.device, load_text_model=False)
    rows=[]
    for position, record in enumerate(records, start=1):
        output=output_dir/f"pair_{position:03d}.png"; row=_evaluate_one(models,record,args.dataset_type,args,output); row["index"]=position; rows.append(row)
        print(f"[{position}/{len(records)}] pair_id={row['pair_id']} zero_mask={row['zero_mask']} components={row['predicted_components']} false_coverage={row['mean_false_mask_fraction']:.4f}", flush=True)
    fieldnames=["index","pair_id","label_type","source_index1","source_index2","algorithm","score","normalized_score","predicted_components","zero_mask","false_positive_mask","false_mask_fraction_line1","false_mask_fraction_line2","mean_false_mask_fraction","image1","image2","output"]
    ...
    summary=_aggregate(rows,args.dataset_type); (output_dir/"summary.json").write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding="utf-8")
