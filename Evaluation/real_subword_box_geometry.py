...
def _geometry(image: Image.Image, target_width: int, target_height: int):
    ...
    if preserve:
        desired_height = max(8, int(round(target_height * _number("ZERO_SHOT_TARGET_INK_HEIGHT_RATIO", 0.72))))
...
def load_line_annotations(image_path, target_width: int, target_height: int) -> BoxAnnotations:
    ...
    with Image.open(image_path) as opened:
        ...
        crop_box, scale_x, scale_y, offset_x, offset_y = _geometry(image, int(target_width), int(target_height))
