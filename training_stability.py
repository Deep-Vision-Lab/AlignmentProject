...
def _save_last_finite(
    ...
    *,
    ...
    """Persist the current finite weights before an unsafe update is applied."""
...
def install_training_stability(train_module, config: dict, job_id: str) -> None:
    """Install finite-loss/gradient/parameter guards on optimized training."""
    ...
    def train_one_epoch(model, text_encoder, criterion, optimizer, scaler, loader):
        ...
        # Refuse to start from an already-corrupted model.  An existing
        # model_last_finite.pth remains untouched in this case.
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            if is_boundary:
                with PROFILER.section("optimizer_and_sync"):
                    # IMPORTANT: synchronize the still-scaled text gradients
                    # first. GradScaler.unscale_ then sees/checks the synchronized
                    # values, so an Inf from any rank cannot bypass scaler.step.
                    coalesced_text_allreduce(
                    ...
                    if gradients_bad:
                        ...
                        optimizer.zero_grad(set_to_none=True)
                        # unscale_ populated GradScaler's found_inf state. update()
                        # lowers the scale and resets it for the next iteration.
                        scaler.update()
                        consecutive_nonfinite += 1
                        if train_module.CTX.is_main:
                            print(
                        ...
                        # Keep the finite forward loss in diagnostics even though
                        # this optimizer update was intentionally skipped.
