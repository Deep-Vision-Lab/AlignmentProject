...
Training exposure is independently balanced along two axes:
- positive vs no_shared rows: 1:1, so sequence-ranking batches almost always
  contain both classes;
- clean vs online-augmented views: 1:2 by default.
...
class BalancedCleanOnlineAugmentedMix(Dataset):
    """Balance class and clean/augmented exposure independently.

    Epoch indices cycle over:
      positive/no_shared x clean/augmented-view slots.
    Shuffling randomizes those slots within batches while preserving exact epoch
    proportions. Augmented datasets generate a fresh stochastic view per access.
    """
    ...
    def __init__(
        ...
        if len(clean_positive) <= 0 or len(clean_negative) <= 0:
            raise ValueError("Joint real training requires positive and no-shared rows.")
        if len(clean_positive) != len(augmented_positive):
            raise ValueError("Positive clean/augmented datasets must have equal length.")
        if len(clean_negative) != len(augmented_negative):
            raise ValueError("No-shared clean/augmented datasets must have equal length.")
        ...
        self.full_cycle = 2 * self.view_cycle
        self.target_length = max(self.full_cycle, int(target_length))
    ...
    def __getitem__(self, index):
        index = int(index)
        view_slot = index % self.view_cycle
        class_slot = (index // self.view_cycle) % 2
        occurrence = index // self.full_cycle
        ...
        return dataset[occurrence % len(dataset)]
...
def build_joint_dataloaders(data_dir):
    ...
    train_dataset = BalancedCleanOnlineAugmentedMix(
        train_positive,
        augmented_positive,
        extra_train,
        augmented_negative,
    ...
    print(
        ...
        f"positive_ratio=0.500 no_shared_ratio=0.500 "
...
def install(base) -> None:
    ...
    # call gives v4 the balanced clean+online-augmented data recipe without
    # duplicating the loss plumbing.
        ...
        config.update(
            {
                ...
                "real_class_balance_positive": 0.5,
                "real_class_balance_no_shared": 0.5,
    ...
    if getattr(base.CTX, "is_main", True):
        print(
            ...
            "positive image-pair contrastive + sequence ranking; balanced positive/"
            "no-shared sampling; online augmentation only.",
            flush=True,
        )
